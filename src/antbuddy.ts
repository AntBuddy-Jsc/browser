...
export class AntBuddy {
  ...
  static init = (config: Config) => {
    return new AntBuddy(config);
  };

  static open = () => window.dispatchEvent(new Event('antbuddy:open'));
  static close = () => window.dispatchEvent(new Event('antbuddy:close'));
  static toggle = () => window.dispatchEvent(new Event('antbuddy:toggle'));
  ...
  setCustomerId = (customerId: string | null) => {
    ...
    window.dispatchEvent(
      new CustomEvent('antbuddy:customer:set', {
        detail: customerId,
      })
    );
  ...
  isValidCustomerId = async (customerId?: string | null) => {
    ...
    try {
      ...
      this.logger.warn('You might be on an older version of AntBuddy.');
